...
#include <windows.h> // used to set window title
...
int main(){
  ...
  SetConsoleTitle("Hollyhock-2 Classpad updater patcher");

  printf("This program modifies the Casio Classpad CP-fx400 updater to add the hollyhock launcher to the calculators firmware.\n");
  printf("This program uses the \"Resource Hacker\".\n");
  printf("This program was written in C and compiled using \"gcc\" from \"minGW\".\n");
  printf("This program uses the libary \"zlib.h\" (mingw32-libz-dev).\n");
  printf("This program uses QuickBMS \"(https://aluigi.altervista.org/quickbms.htm)\" to run the Total Commander plugin Total Observer \"(https://ghisler.ch/board/viewtopic.php?t=42747)\" which is used to extract some of the files.\n");
  printf("\n");
  printf("This program was written by SnailMath (aka Pascal, aka Oberschnecke), and modified by MMKF\n");
  printf("If you have questions, ask the community on Discord: https://discord.com/invite/fWxZXvy\n");
  printf("\n");
  ...
  system("mkdir temp"); //create dir for extracted files, etc

  printf("Checking if Resource Hacker is installed or the portable version is accessable...\n");
  ...
  if( access( "ResourceHacker.exe", R_OK) != -1 ) { 
    ...
    ResourceHacker.exe\n\
    Please ask for help in the Discord server: https://discord.com/invite/fWxZXvy\n\n");
    fgets(str, 3,stdin); return -1;
  }

  printf("\nChecking if updater is present...\n");
  if( 
  ( access("temp/OSupdateDLL_orig.dll", R_OK) != -1 ) &&
//  ( access("LanguageResource.dll", R_OK) != -1 ) &&
  ( access("temp/fxASPI.dll", R_OK) != -1 ) ) {
    printf("  OK - all dlls where already present\n\n");
  } else {

    printf(" dlls not present, extracting from installer...\n");

    system("mkdir temp\\extracted");
    system("quickbms.exe TotalObserver.wcx CAS_color_graphic_OSver2_01_2000_Update.exe temp/extracted");
    SetConsoleTitle("Hollyhock-2 Classpad updater patcher"); // set again because the previous command changes it
    system("start /min msistart.bat");
    sleep(4);
    system("copy \"%temp%/{15B89FD6-8906-4C3B-A271-A4B3A29D545A}\" .\\temp");
    system("move temp\\OSupdateDLL.dll temp\\OSupdateDLL_orig.dll");
    system("taskkill.exe /f /im msiexec.exe");
...
//    ( access("LanguageResource.dll", R_OK) != -1 ) &&
    ( access("temp/fxASPI.dll", R_OK) != -1 ) ) {
      ...
      printf("\n ERROR! The dll files failed to extract! \n For help, ask on Discord: https://discord.com/invite/fWxZXvy \n\n");
      fgets(str, 3,stdin); return -1; 
    }
  }
  ...
  if(access("temp/3070_orig.bin", R_OK) != -1 ){
    ...
    if (reslocation == 1) {
      printf("ext64");
      system("\"\\Program Files\\Resource Hacker\\ResourceHacker.exe\" -open temp/OSupdateDLL_orig.dll -save temp/3070extract.bin -action extract -mask RCData,3070,");
    } else {
      if (reslocation == 2) {
        printf("ext86");
        system("\"\\Program Files (x86)\\Resource Hacker\\ResourceHacker.exe\" -open temp/OSupdateDLL_orig.dll -save temp/3070extract.bin -action extract -mask RCData,3070,");
      } else {
        if (reslocation == 3) {
          printf("inter");
          system("\"ResourceHacker.exe -open temp/OSupdateDLL_orig.dll\" -save temp/3070extract.bin -action extract -mask RCData,3070,");
        } else {
          printf("Something has gone wrong with the reslocation var");
        }
      } 
    } 
    ...
    //open the input file
    //FILE *Fin;
    Fin = fopen("temp/3070extract.bin","rb"); //Read Binary
    if (Fin == NULL) {
    ...
    //FILE *Fout;
    Fout = fopen("temp/3070_orig.bin","wb"); //Write Binary
    if (Fout == NULL) {
...
//Create default mod.txt if not present
  ...
  Fin2 = fopen("temp/3070_orig.bin","rb"); //binary
...
(For hollyhock-2 use version 02.01.2000 !!!)\n\n\n\n\n\n\n\n",mod_ver,bin_ver);
  ...
  //Open output file.
  Fout = fopen("temp/3070_mod.bin","wb"); //binary
  if (Fin == NULL) { //Error
    printf("\n  ERROR! Cannot open the file \"mod.txt\" for writing. I don't know what went wrong...\n");
    fgets(str, 3,stdin); return -1;
  } //output File opened.
...
// needs to be repeated?
  ...
  do{  
    ...
    if (moddone==0){
      while (i<8) { //convert 8 characters //0 and 1 are ignored, i is 2
        ...
        if (ch == '\'') {
          ...
          printf("  ERROR! The file \"mod.txt\" has an error. Wrong Character '%c'(0x%2x) in an address.", ch, ch);
          fgets(str, 3,stdin); return -1;    
        }
    ...
    if (moddone) {
      ...
      moda -= 0x80000000;
      if (moda > 0x14fffff){
        printf("  ERROR! The file \"mod.txt\" was created by an idiot. The firmware is only 0x14ffff bytes long. I can't modify 0x%x!!!\n", moda);
        fgets(str, 3,stdin); return -1;    
      }
    }
  ...
  //open input file
  Fin = fopen("temp/3070_mod.bin","rb"); //Binary mode!!! Windows is stupidoo
  if (Fin == NULL) {
  ...
  Fout = fopen("temp/3070new.bin","wb"); //Binary mode
  if (Fout == NULL) {
...
Now compressing...\n");// ( . input  | output)\n");
  ...
  do {
    ...
    if (strm.avail_in != 0) {
      printf("  ERROR! Zlib didn't eat all of it's food.\n");
      fgets(str, 3,stdin); return -1;
    }
  ...
  (void)fclose(Fin);
  (void)fclose(Fout);
  printf("\n\nSuccessfully packed the firmware into the archive.\n\
  Now modify the dll file and save as \"OSupdate_mod.dll\".\n");

  if (reslocation == 1) {
    system("\"\\Program Files\\Resource Hacker\\ResourceHacker.exe\" -open temp/OSupdateDLL_orig.dll -save temp/OSupdateDLL_mod.dll -res temp/3070new.bin -action addoverwrite -mask RCData,3070,");
  } else {
    if (reslocation == 2) {
      system("\"\\Program Files (x86)\\Resource Hacker\\ResourceHacker.exe\" -open temp/OSupdateDLL_orig.dll -save temp/OSupdateDLL_mod.dll -res temp/3070new.bin -action addoverwrite -mask RCData,3070,");
    } else {
      if (reslocation == 3) {
        system("\"ResourceHacker.exe\" -open temp/OSupdateDLL_orig.dll -save temp/OSupdateDLL_mod.dll -res temp/3070new.bin -action addoverwrite -mask RCData,3070,");
      } else {
        printf("Something has gone wrong with the reslocation var");
      }
    } 
...
In the last step we will modify the binary value in the dll (from 0x3000)\n");
  ...
  Fin = fopen("temp/OSupdateDLL_mod.dll","rb+");
...
");
  ...
  system("start rundll32 .\\temp\\OSupdateDLL_mod.dll,OSUpdate");
